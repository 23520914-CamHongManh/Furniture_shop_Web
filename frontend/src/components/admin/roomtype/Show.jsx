...
const Show = () => {
    ...
    const deleteRoomType = async (id) => {
        ...
        if (confirm("Are you sure you want to delete?")) {
            const res = await fetch(`${apiUrl}/roomtypes/${id}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    'Accept': 'application/json',
                ...
                .then(result => {
                    if (result.status == 200) {
                        const newRoomTypes = roomtypes.filter(roomtype => roomtype.id != id)
            ...
            <div className="container">
                <div className="row">
                    <div className="d-flex justify-content-between mt-5 pb-3">
                        <h4 className="h4 pb-0 mb-0">Room Types</h4>
                        <Link to='/admin/roomtypes/create' className="btn btn-primary">Create</Link>
                    </div>
                    ...
                    <div className="col-md-9">
                        <div className="card shadow">
                            <div className="card-body p-4">
                                ...
                                {
                                    loader == false && roomtypes.length == 0 && <Nostate text="Room Types not found" />
                                }
                                                ...
                                                roomtypes.map(roomtype => {
                                                    return (
                                                        <tr key={`roomtype-${roomtype.id}`}>
                                                            ...
                                                            <td>
                                                                {
                                                                    roomtype.status == 1 ? <span className="badge text-bg-success">Active</span>
                                                                        : <span className="badge text-bg-danger">Block</span>
                                                                }
                                                            </td>
                                                            <td>
                                                                <Link to={`/admin/roomtypes/edit/${roomtype.id}`} className='text-primary'>
                                                                ...
                                                                <Link className='text-danger ms-2' onClick={() => deleteRoomType(roomtype.id)}>
